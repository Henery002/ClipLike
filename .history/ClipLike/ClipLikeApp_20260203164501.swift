    ...
    init(selectionService: SelectionService, onOpenSettings: @escaping () -> Void) {
        ...
        let hostingView = NSHostingView(
            rootView: OverlayView(
                ...
                onSearch: { [weak panel, weak selectionStore] in
                    self.performSearch(using: selectionStore, logger: logger)
                    panel?.orderOut(nil)
                },
                onCopy: { [weak panel, weak selectionStore] in
                    self.performCopy(using: selectionStore, logger: logger)
                    panel?.orderOut(nil)
                },
                onBob: { [weak panel, weak selectionStore] in
                    self.performBob(using: selectionStore, logger: logger)
                    panel?.orderOut(nil)
                },
                onRagHub: { [weak panel] in
                    self.performRagHubShortcut()
                    panel?.orderOut(nil)
                }
            )
        )
