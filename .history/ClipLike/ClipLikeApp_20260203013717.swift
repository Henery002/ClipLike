...
final class SelectionService {
    ...
    private func fetchFocusedElement() -> AXUIElement? {
        ...
        let systemResult = AXUIElementCopyAttributeValue(systemWide, kAXFocusedUIElementAttribute as CFString, &systemValue)
        if systemResult == .success, let systemValue, CFGetTypeID(systemValue) == AXUIElementGetTypeID() {
            return unsafeBitCast(systemValue, to: AXUIElement.self)
        }
        ...
        let appResult = AXUIElementCopyAttributeValue(appElement, kAXFocusedUIElementAttribute as CFString, &appValue)
        if appResult == .success, let appValue, CFGetTypeID(appValue) == AXUIElementGetTypeID() {
            return unsafeBitCast(appValue, to: AXUIElement.self)
        }
        ...
        if windowResult == .success, let windowValue, CFGetTypeID(windowValue) == AXUIElementGetTypeID() {
            let windowElement = unsafeBitCast(windowValue, to: AXUIElement.self)
            var windowFocusedValue: CFTypeRef?
            let windowFocusedResult = AXUIElementCopyAttributeValue(windowElement, kAXFocusedUIElementAttribute as CFString, &windowFocusedValue)
            if windowFocusedResult == .success, let windowFocusedValue, CFGetTypeID(windowFocusedValue) == AXUIElementGetTypeID() {
                return unsafeBitCast(windowFocusedValue, to: AXUIElement.self)
            }
